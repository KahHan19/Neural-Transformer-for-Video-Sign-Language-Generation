...
def draw_skeleton(img,
    ...
    num_keypoints = keypoints_array.shape[1]
...
def draw_openpose(img,
    ...
    for i, ske_info in skeleton_info.items():
        ...
        if i <= 16:
            ...
            polygons = cv2.ellipse2Poly((int(mX), int(mY)),(int(length / 2), int(line_width)),int(angle), 0, 360, 1)
