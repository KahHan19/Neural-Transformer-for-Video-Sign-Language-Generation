...
def calculate_pck(pred, gt, alpha=0.2):
    ...
    # Changes Here where [::3] becomes [::2]
    pred2=[]
    gt2=[]
    for i in range(pred.shape[0]):
        x = [j for j in pred[i][::2]]
        y = [j for j in pred[i][1::2]]
        x2 = [j for j in gt[i][::2]]
        y2 = [j for j in gt[i][1::2]]
        pred2.append([x,y])
        gt2.append([x2,y2])
    pred = np.array(pred2)
    gt = np.array(gt2)
...
def compute_pck_radius(gt, alpha):
    ...
    max_axis_per_keypoint = np.tile(np.expand_dims(max_axis, -1), [1, 134])
